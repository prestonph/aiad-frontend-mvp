...
function newVideo(id: number, status: VideoStatus, name: string, createTimeStr: string, slug: string, scenesData: string = ""): Video {
    return {
        id: id,
        status: status,
        name: name,
    ...
    const handleCreateVideo = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (response.status === 200) {
                ...
                // Add a placeholder while waiting for next call of fetchVideos
            ...
            <div className="grid grid-cols-1 max-w-4xl sm:grid-cols-2 lg:grid-cols-3 gap-6 mt-4">
                {previousVideos.map((video, index) => (
                    <div key={video.id} className="flex flex-col items-center">
                        <div
                            className="relative aspect-video bg-black rounded-lg flex items-center justify-center h-96 w-full">
