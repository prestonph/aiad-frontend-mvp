...
export const PricingCard = ({ price }: Props) => {
	const { name, sub_title, price: mainPrice, price_label, popular, features, action_btn } = price
	...
	return (
		<div className='pt-[40px] px-[30px] pb-[30px] 2xl:p-[50px] rounded-theme relative text-start'>
			...
			<h3 className='text-[40px] xl:text-[50px] font-semibold leading-none mt-[33px]'>
				{mainPrice}{' '}
				<span className='text-[18px] text-secondary font-normal'>/{price_label}</span>
			</h3>
			...
			{action_btn && action_btn.enable && (
				...
				>
					<span className='btn-span text-white' data-text={action_btn.label}>
						{action_btn.label}
					</span>
				</Link>
			)}
