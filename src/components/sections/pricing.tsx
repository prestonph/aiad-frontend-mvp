...
import { PricingCard } from "../elements/pricing-card";
...
export const Pricing = ({ pricing }: Props) => {
  const { title, details, monthly, yearly } = pricing;
  return (
        ...
        >
          <div className="text-center xl:text-start">
            ...
            <TabsList className="mt-[33px] xl:mt-[43px] 2xl:mt-[63px] has_fade_anim">
              <div className="bg-[#121B21] text-white rounded-full relative">
                <TabsTrigger
                  value="monthly"
                  className="px-[38px] py-[22px] text-white text-[16px] leading-none data-[state=active]:bg-theme rounded-full transition-all duration-300"
                >
                  Monthly
                </TabsTrigger>
                <TabsTrigger
                ...
                <div className="hidden md:block absolute bottom-[5px] start-[calc(100%+27px)] w-max">
                  <Image
                    width={76}
                    height={25}
                    src="/Assets/pricing/shape-1.png"
                    alt="shape image"
                  />
                </div>
